...
class DeepfakeAbstractBaseDataset(data.Dataset):
        ...
        for label in dataset_info[dataset_name]:
            ...
            for video_name, video_info in sub_dataset_info.items():
                ...
                if '\\' in frame_paths[0]:
                    ...
                    frame_paths = [p.replace('\\', '/') for p in frame_paths]  
                else:
                    frame_paths = sorted(frame_paths, key=lambda x: int(x.split('/')[-1].split('.')[0]))
    ...
    def load_rgb(self, file_path):
        ...
        """
        file_path = self.config['dataset_root_rgb'] + '/' + file_path
    ...
    def data_aug(self, img, landmark=None, mask=None, augmentation_seed=None):
        ...
        if random.random() < 0:
            img_np = np.asarray(img, dtype=np.float32) / 255.0  # (224,224,3), float32

            amp = np.zeros((3, 224, 224), dtype=np.float32)
            pha = np.zeros((3, 224, 224), dtype=np.float32)
            for c in range(3):
                F = np.fft.fftshift(np.fft.fft2(img_np[..., c]))  # complex128
                amp[c] = np.abs(F).astype(np.float32)
                pha[c] = np.angle(F).astype(np.float32)
            ...
            mask_high = (R > 0.25 * Rmax) & (R < 0.5 * Rmax)


            coords = np.argwhere(mask_high)
            n_remove = int(1 * len(coords))
            idx = np.random.choice(len(coords), n_remove, replace=False)
            mask_rand = np.zeros((H, W), dtype=bool)
            mask_rand[tuple(coords[idx].T)] = True

            amp2 = amp.copy()
            for c in range(3):
                # amp2[c][mask_rand] = np.log1p(amp2[c][mask_rand])
                amp2[c][mask_rand] = 1.1 * amp2[c][mask_rand]
                # amp2[c][mask_rand] = 0


            out = np.zeros((H, W, 3), dtype=np.float32)
            for c in range(3):
                Fc = amp2[c].astype(np.float64) * np.exp(1j * pha[c].astype(np.float64))
                Fc = np.fft.ifftshift(Fc)
                rec = np.fft.ifft2(Fc).real  # float64
                out[..., c] = rec.astype(np.float32)  # -> float32
    ...
    def __getitem__(self, index, no_norm=False):
        ...
        if 'Face2Face' in image_paths:
            FF_label = 1
        elif 'Deepfakes' in image_paths:
            FF_label = 2
        elif 'FaceSwap' in image_paths:
            FF_label = 3
        elif 'NeuralTextures' in image_paths:
            FF_label = 4
        else:
            FF_label = 0
        ...
        return image_tensors, label, landmark_tensors, mask_tensors, FF_label
    ...
    def collate_fn(batch):
        ...
        images, labels, landmarks, masks, fflabels = zip(*batch)

        # Stack the image, label, landmark, and mask tensors
        images = torch.stack(images, dim=0)
        labels = torch.LongTensor(labels)
        fflabels = torch.LongTensor(fflabels)
